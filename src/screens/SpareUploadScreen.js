...
const SpareUploadScreen = ({ navigation }) => {
        ...
        const callback = (response) => {
            if (!response.didCancel && !response.errorCode && response.assets?.length) {
                ...
                if (newImages.length <= 3) {
                    ...
                    showToast('error', 'Limit reached', 'You can only select up to 3 images');
                }
            }
    ...
    const handlePublish = async (actionType = 'save') => {
        ...
        try {
            ...
            formPayload.append('brand_id', formData?.SpareBrandId);
            formPayload.append('description', formData?.Sparedescription);
            formPayload.append('year_of_manufacture_id', formData?.SpareyearId);
            formPayload.append('product_type_id', formData?.spareProductTypeId);
            formPayload.append('price', parseInt(formData?.Spareprice));
            formPayload.append('condition_id', formData?.spareConditionId);
            formPayload.append('category_id', formData?.category_id);
            formPayload.append('model_id', formData?.SparePartNameId);
            formPayload.append('sapre_name', formData?.Sparename);
            formPayload.append('subproduct_type_id', formData?.subproducttypeId);
            formPayload.append('subscription_plan', formData?.subscription_plan);
            formPayload.append('model_other_text', formData?.Sparemodel_other_text);
            formPayload.append('brand_other_text', formData?.Sparebrand_other_text);
            ...
            // 4. Send to appropriate API endpoint
            let apiEndpoint = '';
            let response;

            if (isEditing === true) {
                apiEndpoint = `/api/product/spareRoute/update_spares_before_publish/${formData?.spareproductid}`;
                ...
                apiEndpoint = '/api/product/spareRoute/spares';
            ...
            // 5. Handle Success
            // 5. Handle Success
            if (response.data?.success && response.data?.appCode === 1000) {
                const createdSpare = response.data?.data?.createdSpare || response.data?.data?.updatedSpare;

                showToast('success', 'Success', `Spare ${actionType === 'save' ? 'saved' : 'published'} successfully!`);
                ...
                if (response.data?.code === 'VALIDATION_ERROR' && response.data?.meta?.errors?.length) {
                    const fieldErr = response.data?.meta.errors[0];
                    showToast('error', fieldErr.field, fieldErr.message);
                } else {
                    // ✅ Generic failure message
                    showToast('error', 'Failed', response.data?.message || 'Submission failed');
                }
            }
    ...
    return (
        <BackgroundWrapper>
            ...
            <Modal visible={showConfirmModal} transparent animationType="fade" onRequestClose={() => InteractionManager.runAfterInteractions(() => {
                setShowConfirmModal(false);
            })}>
                <View style={styles.bottomModalOverlay}>
                    <View style={[styles.bottomModal, { backgroundColor: theme.colors.card }]}>
                        <TouchableOpacity style={styles.closeIconWrapper} onPress={() => InteractionManager.runAfterInteractions(() => {
                            setShowConfirmModal(false);
                        })}>
...
export default SpareUploadScreen;